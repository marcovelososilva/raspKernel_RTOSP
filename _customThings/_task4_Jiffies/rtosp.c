...
static int __init rtosp_init(void)
{
	...
	debugfs_create_ulong("rtosp_jiffies", 0666, NULL,
			     &rtosp_jiffies_init); /* ++++++++ */
...
static void __exit rtosp_exit(void)
{
	...
	unsigned int rtosp_jiffies_total =
		jiffies_to_msecs((rtosp_jiffies_end - rtosp_jiffies_init));

	struct dentry *ptr =
		debugfs_lookup("rtosp_jiffies", NULL); /* ++++++++ */
	if (ptr != NULL) /* ++++++++ */
		debugfs_remove(ptr); /* ++++++++ */
