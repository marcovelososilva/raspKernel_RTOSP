...
unsigned long rtosp_jiffies_init = 0;    /* ++++++++ */
#pragma endregion
...
static int __init rtosp_init(void)
{
        // unsigned long rtosp_jiffies = jiffies;  /* ++++++++ */
        ...
        pr_info("Jiffies: %lu", rtosp_jiffies_init);                                 /* ++++++++ */
	debugfs_create_ulong("rtosp_jiffies", 0666, NULL, &rtosp_jiffies_init);      /* ++++++++ */
...
static void __exit rtosp_exit(void)
{
        unsigned long rtosp_jiffies_end = jiffies;
        unsigned int rtosp_jiffies_total = jiffies_to_msecs((rtosp_jiffies_end - rtosp_jiffies_init));

        struct dentry *ptr = debugfs_lookup("rtosp_jiffies", NULL);     /* ++++++++ */
        if(ptr != NULL)                                                 /* ++++++++ */
                debugfs_remove(ptr);                                    /* ++++++++ */
        ...
        cdev_del(&etx_cdev);
        unregister_chrdev_region(dev, 1);
        pr_info("RTOSP - Device Driver took - %u - Jiffies.\n", rtosp_jiffies_total);  /* ++++++++ */
